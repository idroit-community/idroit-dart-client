...
/* (必須) DID情報の最終更新 */
  ...
  @override
  String toString() {
    return 'AllOfVpInfoDidInfo[id=$id, manageUuid=$manageUuid, did=$did, method=$method, label=$label, domainName=$domainName, existPrivateKey=$existPrivateKey, description=$description, user=$user, createdBy=$createdBy, vpInfos=$vpInfos, groups=$groups, createdAt=$createdAt, updatedAt=$updatedAt, ]';
  }

  AllOfVpInfoDidInfo.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    id = json['id'];
  ...
  static List<AllOfVpInfoDidInfo> listFromJson(List<dynamic> json) {
    return json == null ? new List<AllOfVpInfoDidInfo>() : json.map((value) => new AllOfVpInfoDidInfo.fromJson(value)).toList();
  }

  static Map<String, AllOfVpInfoDidInfo> mapFromJson(Map<String, Map<String, dynamic>> json) {
    var map = new Map<String, AllOfVpInfoDidInfo>();
    if (json != null && json.length > 0) {
      json.forEach((String key, Map<String, dynamic> value) => map[key] = new AllOfVpInfoDidInfo.fromJson(value));
    }
    return map;
  }
}
